  ...
  <% pageContext.setAttribute("searchTime", Long.valueOf((System.currentTimeMillis() - startTime))); %>
	<c:if test="${searchMap['listApproxSize'] > 0 || moduleMap['listTotalSize'] > 0}">
        ...
        <c:if test="${searchMap['listApproxSize'] > 0 && searchMap['listApproxSize'] < 2147483647 || moduleMap['listTotalSize'] < 2147483647}">
          ...
          <div class="searchDetails"> <h3><fmt:message key="${messKey}"><fmt:param value="${fn:escapeXml(param[termKey])}"/><fmt:param value="${searchMap['listApproxSize'] > 0 ? searchMap['listApproxSize'] : moduleMap['listTotalSize']}"/><fmt:param value="${searchTime}"/></fmt:message></h3></div>
        </c:if>
